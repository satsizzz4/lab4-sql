...
select category.cat_id,category.cat_name,min(t3.mi_price as Min_Price from category
inner join 
 ...
 */
 select 'order' pricing_id , rating.ORD_ID,rating.RAT_RATSTARS FROM 'order' inner join rating.'ord_ui' = 'order'. ord_id;
 
select report.supp_id,report.supp_name,report.Average,
case
When report.Average=5 THEN 'Execellent Serivce'
When report.Average>4 THEN 'Good Serivce'
When report.Average>2 THEN 'Average Serivce'
ELSE 'Poor Service'
END AS Type _of_service from
(select final.supp_id, supplier.supp_name,final.Average from
(select test2.supp_id,sum(test2.rat_rat_ratstars)/count(test2.rat_ratstars) as Average from
(select supplier_pricing.supp_id,test.ORD_ID,test.RAT_RATSTARS from supplier_pricing inner join

(select 'order'.pricing_id, ratingORD_ID,rating.RAT_RATSTARS FROM 'ORDER'
INNER JOIN
rating
